...
- **F2: no `^C` echo on Ctrl-C foreground.** Pressing Ctrl-C in slash kills the foreground job and returns the prompt with `[130]` (correct), but the kernel never echoed `^C` first (zsh and bash both do). Diagnosis: tty `ECHOCTL` likely cleared by the `enterRawMode`/`leaveRawMode` round trip, or our `shell_termios` baseline is missing it. Fix path: explicitly ensure `ECHOCTL|ISIG` are on when handing the tty to a foreground job (the standard "shell mode" → "user mode" termios distinction). **Deferred.**
