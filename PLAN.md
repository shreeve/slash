...
You write **commands** and connect them into **pipelines**; Slash parses each line into a `Shape`, lowers it into a `Program`, and runs it as a `Job` you can inspect and control. The first two stages are how Slash thinks; the last stage is what you see.
...
├── src/
│   ├── parser.zig         # generated by nexus from slash.grammar (do not edit)
│   ├── slash.zig          # lang module for the generated parser (Tag, keywordAs, optional Lexer wrapper)
